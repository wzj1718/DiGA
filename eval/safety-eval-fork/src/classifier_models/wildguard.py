...
DEFAULT_WILDGUARD_MODEL_PATH = "./models/allenai/wildguard"
HF_WILDGUARD_MODEL_NAME = "allenai/wildguard"


def _get_wildguard_model_name_or_path() -> str:
    model_path = os.environ.get("WILDGUARD_MODEL_PATH", DEFAULT_WILDGUARD_MODEL_PATH)
    if os.path.exists(model_path):
        return model_path
    return HF_WILDGUARD_MODEL_NAME
...
class WildGuard(WildGuardBase):
    MODEL_NAME = HF_WILDGUARD_MODEL_NAME
    ...
    def __init__(
        ...
        self.model_name_or_path = _get_wildguard_model_name_or_path()
        if ephemeral_model:
            self.model = None
        else:
            self.model = LLM(model=self.model_name_or_path, tokenizer_mode="slow")
